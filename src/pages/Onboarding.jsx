    ...
    <div className="max-w-3xl mx-auto px-6 py-12">
      <Card className="p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <div className="flex gap-2 mb-2">
              ...
              <Button type="button" size="sm" onClick={() => { 
                if(skillInput) setForm({ ...form, skills: [...form.skills, skillInput] }); 
                setSkillInput(''); 
              }}>Add</Button>
            </div>
            <div className="flex flex-wrap gap-2">
              {form.skills.map(s => (
                <span key={s} className="bg-purple-100 text-purple-700 px-3 py-1 rounded-full text-sm">{s}</span>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2 mb-2">
              <input 
                ...
                placeholder="e.g., Web Dev, AI, UX" 
              />
              <Button type="button" size="sm" onClick={() => { 
                if(interestInput) setForm({ ...form, interests: [...form.interests, interestInput] }); 
                setInterestInput(''); 
              }}>Add</Button>
            </div>
            <div className="flex flex-wrap gap-2">
          ...
          <Button type="button" onClick={handleAISuggestions} variant="secondary">
          ...
          {suggestions && (
            <div className="bg-purple-50 rounded-lg p-4">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <p className="text-sm font-medium">You can help with:</p>
                  <ul className="text-sm text-gray-600">
                    {suggestions.canHelp.slice(0,3).map(s => <li key={s}>• {s}</li>)}
                  </ul>
                </div>
                <div>
                  <p className="text-sm font-medium">You may need help with:</p>
                  <ul className="text-sm text-gray-600">
                    {suggestions.needHelp.slice(0,3).map(s => <li key={s}>• {s}</li>)}
                  </ul>
                </div>
              </div>
            </div>
